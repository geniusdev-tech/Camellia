...
def login():
    ...
    try:
        ...
        if not user.is_active:
            # Consistent 401 response to prevent account enumeration
            return jsonify({'success': False, 'msg': 'Credenciais inválidas'}), 401
...
def login_mfa():
    data = request.json
    code = data.get('code')
    # Strictly use user_id from session to prevent account probing
    ...
    try:
        ...
        if not user or not user.mfa_secret_enc:
             # Consistent 401 response and generic message
